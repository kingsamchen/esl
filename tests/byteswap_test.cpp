...
TEST_CASE("reverse bytes order for a 16-bit integer") {
    SUBCASE("for unsigned") {
        std::uint16_t n{0x1234}; // NOLINT(readability-magic-numbers)
        CHECK_EQ(esl::byteswap(n), std::uint16_t{0x3412});
    }

    SUBCASE("for signed") {
        std::int16_t n{0x77ff}; // NOLINT(readability-magic-numbers)
...
TEST_CASE("reverse bytes order for a 32-bit integer") {
    SUBCASE("for unsigned") {
        std::uint32_t n{0xdeadbeef}; // NOLINT(readability-magic-numbers)
        CHECK_EQ(esl::byteswap(n), std::uint32_t{0xefbeadde});
    }

    SUBCASE("for signed") {
        std::int32_t n{0x01234567}; // NOLINT(readability-magic-numbers)
...
TEST_CASE("reverse bytes order for a 64-bit integer") {
    SUBCASE("for unsigned") {
        std::uint64_t n{0xdeadbeefbaddcafe}; // NOLINT(readability-magic-numbers)
        CHECK_EQ(esl::byteswap(n), std::uint64_t{0xfecaddbaefbeadde});
    }

    SUBCASE("for signed") {
        std::int64_t n{0x0123456789abcdef}; // NOLINT(readability-magic-numbers)
